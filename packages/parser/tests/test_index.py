...
from nlv.index import IndexResult, run_index
...
class TestMapJsonOutput:
    ...
    def test_map_json_deterministic(self, tmp_path: Path) -> None:
        """Running index twice on the same project produces identical map.json."""
        _create_python_project(tmp_path)
        run_index(tmp_path)
...
class TestSummaryFormatting:
    ...
    def test_summary_single_file(self, tmp_path: Path) -> None:
        ...
        result = run_index(tmp_path)
        assert "1 file" in result.summary or "1 files" in result.summary
...
class TestErrorHandling:
    ...
    def test_empty_directory_summary(self, tmp_path: Path) -> None:
        ...
        result = run_index(tmp_path)
        assert "0 files" in result.summary or "no" in result.summary.lower()
